...
class Pengguna extends CI_Controller {
	...
	public function tambah()
	{
		...
		$this->form_validation->set_rules('namapengguna', 'Nama Pengguna', 'required|min_length[5]|is_unique[pengguna.nama_pengguna]');
		$this->form_validation->set_rules('katasandi', 'Kata Sandi', 'required|min_length[5]');
		...
		if($this->form_validation->run() == FALSE){
			$this->template->load('template', 'pengguna/pengguna_form_tambah');
		} else {
	...
	public function ubah($id)
	{
		...
		if($this->form_validation->run() == FALSE){
			...
			if($query->num_rows() > 0){
				$data['row'] = $query->row();
				$this->template->load('template', 'pengguna/pengguna_form_ubah', $data);
			} else {
